...
class TshirtConfiguration(admin.ModelAdmin):
    ...
    list_display = ['get_image','name' , 'slug' , 'descount']
    list_editable = ['descount']
    sortable_by = ['name']
    list_filter = ['descount']
...
class CartConfiguration(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Carty Info", {"fields" : ('user' , 'get_tshirt' , 'get_sizeVariant' , 'quantity')}),
    )
...
class OrderConfiguration(admin.ModelAdmin):
    ...
    readonly_fields = ('user','phone','shipping_address','user','total','payment_method','payment','payment_request_id','payment_id','payment_status')
